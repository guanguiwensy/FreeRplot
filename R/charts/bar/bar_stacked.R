...
chart_def <- list(id = "bar_stacked", name = "堆叠柱状图", name_en = "Stacked Bar Chart", 
    plot_fn = function (data, options = list()) 
    {
        ...
        if (lbls) {
            p <- p + ggplot2::geom_text(ggplot2::aes(label = round(y, 
                1), group = group), position = ggplot2::position_stack(vjust = 0.5), 
                size = lsz, color = "white")
        }
        p <- .bar_orient(p, orient)
        apply_theme(p + ggplot2::labs(x = x_col, y = y_col), 
            options)
    }, category = "柱图家族", description = "各分组数值叠加为整柱，同时展示总量与组成结构", 
    best_for = "总量趋势与构成分析、多组时序堆叠", 
